...
class RedhenDedupeMergeForm extends FormBase {
  ...
  public function redhenDedupeMergeFormCallback(array &$form, FormStateInterface $form_state) {
    ...
    $ajax_response->addCommand(new ReplaceCommand('#redhen_dedupe_merge_data', $this->renderer->render($form['merge_data'])));
  ...
  private function redhenDedupeOptionLabel(Contact $contact, $field_name) {
    ...
    $display = $this->renderer->render($render);
        ...
        foreach ($related_entities as $entity_type) {
          switch ($entity_type) {
            ...
            case 'redhen_membership':
              ...
              $query->condition('entity_type', 'redhen_contact');
              $query->condition('entity_id', $contact_id);
              $result = $query->accessCheck(FALSE)->execute();
              if (!empty($result)) {
                $rel_entities = \Drupal::entityTypeManager()->getStorage($entity_type)->loadMultiple($result);
