...
class ConnectionAddController extends ControllerBase {
  ...
  public function add(Request $request) {
    ...
    if (count($types) === 0) {
      return [
        '#markup' => $this->t('You have not created any Connection types yet. @link to add a new type.', [
          '@link' => Link::createFromRoute($this->t('Go to the type creation page'), 'entity.redhen_connection_type.add_form')->toString(),
        ]),
      ];
    }
    ...
    if (count($types) === 0) {
      return [
        '#markup' => $this->t('You have not created any Connection types for the %bundle bundle yet. @link to add a new type.', [
