...
class Contact extends ContentEntityBase implements ContactInterface {
  ...
  */
  ...
  public static function baseFieldDefinitions(EntityTypeInterface $entity_type) {
    $config = \Drupal::config('redhen_contact.settings')->getRawData();
    if (empty($config)) {
      // Load our default configurations from the yml file.
      $file = \Drupal::service('extension.list.module')->getPath('redhen_contact') . '/config/install/redhen_contact.settings.yml';
      $raw = file_get_contents($file);
      if (empty($raw)) {
        throw new \RuntimeException(sprintf('Config file not found at %s', $file));
      }
      $config = Yaml::decode($raw);
      if (!is_array($config)) {
    ...
    $required_names = $config['required_properties'];
    ...
    $fields['email'] = BaseFieldDefinition::create('email')
      ...
      ->setDisplayConfigurable('view', TRUE)
      ->setRevisionable(TRUE)
      ->setRequired($config['valid_email']);
